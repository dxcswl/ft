...
class Ueditor{
	...
	public function __construct($uid = ''){
        ...
        $fj_ueditor = \think\Env::get('app_path').\think\facade\Config::get('fj_ueditor');
        $fj_ueditor = $fj_ueditor?$fj_ueditor:\think\Env::get('root_path') . 'vendor/'.'"dxcswl/ft/src/org/Ueditor/ueditor.json"';
		\Vin\FileStorage::connect();
		//导入设置
		$CONFIG = json_decode(preg_replace("/\/\*[\s\S]+?\*\//", "", file_get_contents($fj_ueditor)), true);
	...
	private function uploadFile($config,$fieldName){
		...
		$dir_ml = $this->rootpath.$this->getFullPaths($config['pathFormat']);
		$dir_url = ROOT_PATH . 'public' .$dir_ml;
		$info = $file->rule('uniqid')->move($dir_url);
		if(!$info){
			$data = array(
				"state"=>$file->getError(),
			);
	 ...
	 * @return string
	 */
	private function getFullPaths($path)
	{
		//替换日期事件
		$t = time();
		$d = explode('-', date("Y-y-m-d-H-i-s"));
