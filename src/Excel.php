...
class Excel {
	//配置
	static public $config = array('remove' => false,        //是否上传后删除文件
		'filename' => 'filename', //文件名称
		'rootpath' => './uploads', //上传主目录
		'savepath' => '/Files/Excel/',//上传子目录
	...
	static public function export($data, $fields = null, $savename = null, $title = 'Sheet1', $suffix = 'xlsx') {
		//导出数据
		$data = !empty(self::$config['data']) ? self::$config['data'] : (!empty($data) ? $data : exit('导出数据为空！'));
		//第一列字段标题
		$fields = !empty(self::$config['fields']) ? self::$config['fields'] : (!empty($fields) ? $fields : null);
		//文件名称
		$savename = !empty(self::$config['savename']) ? self::$config['savename'] : (!empty($savename) ? $savename : date('Y-m-d_H_I_s'));
		//表名称
		...
		/*导入phpExcel核心类 */
		Vendor("PHPExcel.PHPExcel");
		$suffix = 'xlsx' ? Vendor("PHPExcel.PHPExcel.Writer.Excel2007") : Vendor("PHPExcel.PHPExcel.Writer.Excel5");
	...
	public function exportFile(array $title, array $data, $outputFileName = '') {
		...
		foreach ($data as $key => $value) {
			for ($m = 0; $m <= $i; $m++) {
				...
				//$objActSheet->setCellValue(chr($m+65).($key+2), $value[$arrKeys[$m]]);
			}
		}
		header("Content-Type: application/force-download");
		header("Content-Type: application/octet-stream");
