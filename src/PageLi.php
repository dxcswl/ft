...
class PageLi{
    ...
    public function init($totalRows, $listRows = 20, $parameter = array()) {
        $this->p = 'page'; //设置分页参数名称
        /* 基础设置 */
        $this->totalRows = $totalRows; //设置总记录数
        $this->listRows = $listRows; //设置每页显示行数
    ...
    public function show() {
        ...
        if(isset($_GET['page'])) { //正则替换
            $this->url = '/' . $_SERVER['PATH_INFO'] . '?' . preg_replace("/page=[1-9][0-9]{0,2}|&page=[1-9][0-9]{0,2}/s", $this->p . '=' . urlencode('[PAGE]'), $this->url);
        } else {
            if($this->url) { //处理不同位置
                $this->url = '/' . $_SERVER['PATH_INFO'] . '?' . $this->url . '&' . $this->p . '=' . urlencode('[PAGE]');
            } else {
                $this->url = '/' . $_SERVER['PATH_INFO'] . '?' . $this->p . '=' . urlencode('[PAGE]');
            }
        }

        //$this->url                 = url(config('MODULE_NAME').'/'.config('CONTROLLER_NAME').'/'.config('ACTION_NAME'), $this->parameter);
        ...
        for($i = 1; $i <= $this->rollPage; $i++) {
            ...
            if($page > 0 && $page != $this->nowPage) {
                ...
                if($page > 0 && 1 != $this->totalPages) {
                    $link_page .= '<li><a class="current active">' . $page . '</a></li>';
                }
            }
        }
