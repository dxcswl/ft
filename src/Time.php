...
class Time {
    ...
    public static function yesterday() {
        $yesterday = date('d') - 1;
        return [mktime(0, 0, 0, date('m'), $yesterday, date('Y')), mktime(23, 59, 59, date('m'), $yesterday, date('Y'))];
    }
    ...
    public static function lastWeek() {
        $timestamp = time();
        return [strtotime(date('Y-m-d', strtotime("last week Monday", $timestamp))), strtotime(date('Y-m-d', strtotime("last week Sunday", $timestamp))) + 24 * 3600 - 1];
    }
     ...
     * @return array
     */
    public static function month($type = 0) {
        list($y, $m, $t) = explode('-', date('Y-m-t'));
        if($type == 0) {
            return [mktime(0, 0, 0, $m, 1, $y), mktime(23, 59, 59, $m, $t, $y)];
        } else if($type == 1) {
            return mktime(0, 0, 0, $m, 1, $y);
        } else if($type == 2) {
            return mktime(23, 59, 59, $m, $t, $y);
         }
    ...
    public static function lastYear() {
        $year = date('Y') - 1;
        return [mktime(0, 0, 0, 1, 1, $year), mktime(23, 59, 59, 12, 31, $year)];
    }
    ...
    public static function dayToNow($day = 1, $now = true) {
        ...
        return [mktime(0, 0, 0, date('m'), date('d') - $day, date('Y')), $end];
    }
